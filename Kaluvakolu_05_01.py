...
from keras.layers import Conv2D, LeakyReLU, Dropout, Flatten, Dense, BatchNormalization, Conv2DTranspose, Reshape
...
Returns:
    ...
    # Add the LeakyReLU activation function
    model.add(LeakyReLU())
...
def build_generator():
    ...
    # Add the Dense layer
    model.add(Dense(units=392, use_bias=False, input_shape=(100,)))
    ...
    model.add(Conv2DTranspose(filters=8, kernel_size=(5, 5),
    ...
    # Add the Conv2DTranspose layer
    model.add(Conv2DTranspose(filters=16, kernel_size=(5, 5),
    ...
    # Add the Conv2DTranspose layer
    model.add(Conv2DTranspose(filters=1, kernel_size=(5, 5), strides=(
...
class DCGAN(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Generate noise vector from a uniform distribution
        noise = tf.random.uniform([batch_size, 100, 1])

        # Use a GradientTape to record the operations for automatic differentiation
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            combined_images_labels = tf.concat(
                [tf.ones([batch_size]), tf.zeros([batch_size])], axis=0)

            # Compute the discriminator predictions on the combined batch
            R_img_pred = self.discriminator(data, training=True)

            F_img_pred = self.discriminator(generated_images, training=True)

            b_img_pred = self.discriminator(combined_images, training=True)

            # Compute the discriminator loss using the binary cross entropy loss function
            # method 1 combine real and fake imgs and then find d_loss
            # d_loss = self.loss_fn(combined_images_labels, b_img_pred)

            # Compute the generator loss using the binary cross entropy loss function
            g_loss = self.loss_fn(tf.ones_like(F_img_pred), F_img_pred)

            # Compute the discriminator loss using the binary cross entropy loss function
            # method 2 find loss on real and fake imgs and then find avg of d_loss
            R_img_loss = self.loss_fn(tf.ones_like(R_img_pred), R_img_pred)
            F_img_loss = self.loss_fn(tf.zeros_like(F_img_pred), F_img_pred)
            d_loss = (R_img_loss + F_img_loss)/2

        # Get the gradients of` the generator and discriminator losses with respect to their trainable variables
